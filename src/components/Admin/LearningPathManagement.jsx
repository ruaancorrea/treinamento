...
import { Plus, Edit, Trash2, ListOrdered, Loader2 } from 'lucide-react';
...
const LearningPathManagement = () => {
    ...
    const loadData = async () => {
        setIsLoading(true);
        const pathsData = await getAllData('trilhas');
        const trainingsData = await getAllData('treinamentos');
        const departmentsData = await getAllData('departamentos');
        setPaths(pathsData);
    ...
    const handleSubmit = async (e) => {
        ...
        if (editingPath) {
            await updateData('trilhas', editingPath.id, formData);
            toast({ title: "Trilha atualizada com sucesso!" });
        } else {
            await addData('trilhas', formData);
    ...
    const handleDelete = async (pathId) => {
        if (window.confirm("Tem certeza que deseja remover esta trilha?")) {
            await deleteData('trilhas', pathId);
            await loadData();
